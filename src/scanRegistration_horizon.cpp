...
int main(int argc, char** argv)
{
  ...
  pubLaserCloud = node->create_publisher<sensor_msgs::msg::PointCloud2>("livox_cloud", rclcpp::QoS(100));
  pubCornerPointsSharp = node->create_publisher<sensor_msgs::msg::PointCloud2>("laser_cloud_sharp", rclcpp::QoS(100));
  pubSurfPointsFlat = node->create_publisher<sensor_msgs::msg::PointCloud2>("laser_cloud_flat", rclcpp::QoS(100));
  auto subLaserCloud = node->create_subscription<sensor_msgs::msg::PointCloud2>("livox_pcl0", 100, laserCloudHandler);
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}